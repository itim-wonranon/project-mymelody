...
<div class="container py-5" style="position: relative; z-index: 1;">
    ...
    <div class="text-center mb-5">
        <h6 class="text-primary fw-bold text-uppercase mb-2" style="letter-spacing: 2px; font-size: 0.85rem;">Discover
            Talents</h6>
        <h1 class="fw-bold text-white display-5 mb-4">ค้นหานักดนตรีที่คุณต้องการ</h1>
        <p class="text-secondary mb-4 mx-auto" style="max-width: 900px;">ค้นหาศิลปิน วงดนตรี
            หรือดีเจที่ตรงกับสไตล์และงบประมาณของคุณ เพื่อทำให้งานของคุณสมบูรณ์แบบที่สุด</p>

        <!-- Horizontal Search Form -->
        <div class="card bg-dark bg-opacity-50 border border-secondary border-opacity-25 rounded-4 p-4 shadow-lg mx-auto"
            style="max-width: 900px; backdrop-filter: blur(10px);">
            <form action="search.php" method="GET" id="searchForm">
                <div class="row g-3 text-start">
                    ...
                    <div class="col-md-9">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="artist_name"
                                ...
                                value="<?php echo htmlspecialchars($artist_name); ?>"
                                placeholder="ค้นหาชื่อศิลปิน, วงดนตรี, หรือแนวเพลง (เช่น Pop, Rock)...">
                        </div>
                    </div>
                    ...
                    <div class="col-12 mt-3">
                        <a class="text-decoration-none small text-primary fw-bold" data-bs-toggle="collapse"
                            href="#advancedFilters" role="button" aria-expanded="false" aria-controls="advancedFilters">
                    ...
                    <div class="collapse col-12 mt-3" id="advancedFilters">
                        <div
                            class="row g-3 px-3 pb-3 pt-2 bg-black bg-opacity-25 rounded-3 border border-secondary border-opacity-10">
                            <!-- Location -->
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="band_type" class="form-select bg-dark text-white border-secondary">
                                    ...
                                    <option value="solo" <?php echo $band_type === 'solo' ? 'selected' : ''; ?>>
                                        ศิลปินเดี่ยว (Solo)</option>
                                    <option value="duo" <?php echo $band_type === 'duo' ? 'selected' : ''; ?>>ศิลปินคู่
                                        (Duo)</option>
                                    <option value="band" <?php echo $band_type === 'band' ? 'selected' : ''; ?>>วงดนตรี
                                        (Band)</option>
                                </select>
                            </div>
                            ...
                            <!-- Max Price -->
                            <div class="col-md-3">
                                <label class="form-label text-light small fw-bold mb-1">งบสูงสุด (บาท)</label>
                                <input type="text" name="price_max" class="form-control bg-dark text-white border-secondary" value="<?php echo htmlspecialchars($price_max); ?>" placeholder="ไม่จำกัด">
                            </div>

                            <!-- Rating -->
                            <div class="col-md-3">
                                <label class="form-label text-light small fw-bold mb-1 d-flex justify-content-between">
                                    <span>คะแนนรีวิวขั้นต่ำ</span>
                                    <span class="text-warning"><span id="ratingValue"><?php echo !empty($rating_min) ? htmlspecialchars($rating_min) : '0'; ?></span> <i class="fas fa-star" style="font-size: 0.8rem;"></i></span>
                                </label>
                                <div class="d-flex align-items-center" style="height: 38px;">
                                    <input type="range" name="rating_min" class="form-range m-0" min="0" max="5" step="0.1"
                                        value="<?php echo !empty($rating_min) ? htmlspecialchars($rating_min) : '0'; ?>"
                                        id="ratingSlider">
                                </div>
                            </div>
                        </div>
    ...
    <?php if ($has_searched): ?>
        ...
        <div
            ...
            <h5 class="text-white mb-0">ผลลัพธ์การค้นหา <span
                    class="badge bg-primary rounded-pill ms-2"><?php echo count($results); ?></span></h5>
        ...
        <?php if (count($results) > 0): ?>
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4 g-4">
                <?php foreach ($results as $musician): ?>
                    <div class="col">
                        <div class="artist-card h-100 d-flex flex-column rounded-4 border border-secondary border-opacity-10 overflow-hidden position-relative hover-lift"
                            style="background: linear-gradient(180deg, #1e1e2d 0%, #151521 100%); box-shadow: 0 10px 30px rgba(0,0,0,0.5); transition: all 0.3s ease;">

                            <!-- Gradient Cover -->
                            <div class="artist-cover position-relative"
                                style="height: 120px; background: linear-gradient(135deg, #ff7eb3 0%, #ff758c 100%);">
                                <div class="position-absolute w-100 h-100"
                                    style="background: linear-gradient(to bottom, rgba(21,21,33,0) 0%, rgba(21,21,33,1) 100%); top: 0; left: 0;">
                                </div>
                            </div>

                            <!-- Avatar overlapping cover -->
                            <div class="artist-avatar-wrapper text-center"
                                style="margin-top: -60px; position: relative; z-index: 2;">
                                <?php
                                ...
                                ?>
                                <img src="<?php echo htmlspecialchars($img_src); ?>" class="rounded-circle shadow-lg"
                                    style="width: 120px; height: 120px; object-fit: cover; border: 4px solid #151521; background: #1a1a2e;"
                                    alt="Profile">
                            </div>
                            ...
                            <div class="card-body text-center d-flex flex-column pt-3 pb-4 px-4">
                                ...
                                ?>
                                <h4 class="fw-bolder text-white text-truncate mb-2"
                                    title="<?php echo htmlspecialchars($display_name); ?>" style="letter-spacing: 0.5px;">
                                    <?php echo htmlspecialchars($display_name); ?>
                                </h4>

                                <div class="mb-3">
                                    <span class="badge rounded-pill"
                                        style="background: linear-gradient(45deg, #8a2387, #e94057); color: #fff; font-weight: 500; padding: 6px 16px; letter-spacing: 0.5px; box-shadow: 0 4px 10px rgba(233, 64, 87, 0.3);">
                                ...
                                <div class="d-flex justify-content-center align-items-center mb-4">
                                    <div class="me-2" style="color: #ffc107;">
                                        ...
                                        for ($i = 1; $i <= 5; $i++) {
                                            if ($score >= $i)
                                                echo '<i class="fas fa-star" style="filter: drop-shadow(0 0 5px rgba(255,193,7,0.5));"></i>';
                                            else if ($score >= $i - 0.5)
                                                echo '<i class="fas fa-star-half-alt" style="filter: drop-shadow(0 0 5px rgba(255,193,7,0.5));"></i>';
                                            else
                                ...
                                <div class="text-start mt-auto w-100 mx-auto" style="max-width: 250px;">
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="rounded-circle d-flex justify-content-center align-items-center me-3"
                                            style="width: 32px; height: 32px; min-width: 32px; background: rgba(230, 133, 255, 0.1);">
                                    ...
                                    <div class="d-flex align-items-center mb-3">
                                        <div class="rounded-circle d-flex justify-content-center align-items-center me-3"
                                            style="width: 32px; height: 32px; min-width: 32px; background: rgba(255, 75, 75, 0.1);">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="rounded-circle d-flex justify-content-center align-items-center me-3"
                                            style="width: 32px; height: 32px; min-width: 32px; background: rgba(0, 230, 118, 0.1);">
                                ...
                                <a href="musician_profile.php?id=<?php echo $musician['user_id']; ?>"
                                    class="btn w-100 rounded-pill mt-4 py-2 fw-bold text-white shadow-lg"
                                    style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; transition: all 0.3s ease;"
                                    onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 8px 20px rgba(118, 75, 162, 0.6)';"
                                    onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 10px rgba(118, 75, 162, 0.4)';">
                                    ดูโปรไฟล์ศิลปิน
                                </a>
                            </div>
                        </div>
        ...
        <?php else: ?>
            <!-- Empty State -->
            <div
                class="d-flex flex-column align-items-center justify-content-center text-center py-5 my-5 bg-dark bg-opacity-25 rounded-4 border border-secondary border-opacity-25">
                <div class="mb-4 p-4 rounded-circle" style="background: rgba(196, 113, 237, 0.1);">
                    <i class="fas fa-search-minus fa-3x text-primary"
                        style="filter: drop-shadow(0 0 10px rgba(196, 113, 237, 0.5));"></i>
                </div>
                <h4 class="text-white fw-bold mb-2">ไม่พบศิลปินที่ตรงกับเงื่อนไข</h4>
                <p class="text-secondary mb-4 max-w-md mx-auto">ลองปรับเปลี่ยนคำค้นหา ขยายช่วงราคา หรือลดเงื่อนไขตัวกรองลง
                    เพื่อให้พบผลลัพธ์ที่หลากหลายขึ้น</p>
